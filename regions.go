...
// This is the Region type structure, holding an unique code,
// and the region name. The code is a 4 characters string, made
// of the concatenation of the ISO3661 codes for the country and 
// region. For example, "FRA8", is Ile-de-France ("A8") in 
// France ("FR") :
// 	{ "FRA8", "Ile-de-France" }
type Region struct {
	Code string	
...
// This is the type for a BTree holding all the known regions 
// in memory
type Regions btree.BTree

// Implements String() function to *Region type, so it
// implements the Stringer interface an can be Println()
func (region *Region) String() string {
	return fmt.Sprintf("Region.Code=%q, Region.Name=%q",
		region.Code, region.Name)
}


// Implements the Item interface from btree package for
// the Region type, so we can use them in a btree.
// Less tests whether the current item is less than the given argument.
func (region Region)Less(than btree.Item) bool {
	return region.Code < than.(Region).Code
}


// Load all the known regions in memory as a BTree. The regions
// information are read from the local const regions_list defined
// at the end of this file.
func LoadRegions() (*Regions, error) {
	r := csv.NewReader(strings.NewReader(regions_list))
	r.FieldsPerRecord = -1
    r.Comma = ','
    t := btree.New(4)
    for {
    	...
    	if err != nil {
    		log_geolocip.Err(fmt.Sprintf("Regions error %v", err))
    		break
    	}
		...
		// Use only lines with 3 values
	   	if len(values) == 3 {
	   		code := fmt.Sprintf("%s%s", values[0], values[1])
	   		t.ReplaceOrInsert(Region{ code, values[2] })
	   	}
    }
...
// Returns Region structure matching a given region code
func (regions *Regions)Get(region_code string) *Region {
	tree := (*btree.BTree)(regions)
	item := tree.Get(Region{region_code, ""})
	if item != nil {
		region := item.(Region)
		return(&region)
	} else {
...
// Local constant holding the regions info
const (
	regions_list = `AD,02,"Canillo"
AD,03,"Encamp"
AD,04,"La Massana"
AD,05,"Ordino"
